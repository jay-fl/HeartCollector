...
export class Game extends Phaser.Scene {
	...
	create() {
		...
		this.info = this.add.text(480, 16, '', {
			font: '32px Arial',
			fill: '#000000',
		})
		...
		this.enemy = this.physics.add.sprite(400, 320, 'enemy').setScale(0.1)
		this.enemy1 = this.physics.add.sprite(250, 77, 'enemy').setScale(0.1)
		this.enemy2 = this.physics.add.sprite(118, 198, 'enemy').setScale(0.1)
		this.enemy3 = this.physics.add.sprite(200, 300, 'enemy').setScale(0.1)
		...
		this.enemy1.setBounce(0.3)
		this.enemy1.anims.play('fire_walk')

		this.enemy2.setBounce(0.3)
		this.enemy2.anims.play('fire_walk')

		this.enemy3.setBounce(0.3)
		...
		this.hearts = this.physics.add.group({
			...
			repeat: 13,
			setXY: {
				x: Phaser.Math.FloatBetween(101, 150),
				y: Phaser.Math.FloatBetween(0, 250),
				stepX: 42,
			},
		})
		...
		this.scoreText = this.add.text(130, 16, 'Score: 300', {
			font: '32px Arial',
			fill: '#000',
		})
	...
	update() {
		...
		if (this.cursors.left.isDown) {
			...
			if (!this.setpSound.isPlaying) this.setpSound.play()
		...
		if (this.score < 132 && this.score >= 90) {
			...
			this.moveEnemy(this.enemy1, 120, 220, 570)
			this.moveEnemy(this.enemy2, 120, 115, 678)
		}
		...
		if (this.hearts.countActive(true) === 0) {
			this.hearts.children.iterate(function (child) {
				child.enableBody(
					true,
					child.x,
					Phaser.Math.FloatBetween(0, 250),
					true,
					true
				)
			})
